...
onMounted(async () => {
  try {
    ...
    uiStore.showToast('加载群组设置失败', 'error')
  } finally {
...
async function copyGroupNumber() {
  const num = group.value?.groupNumber
  if (!num) return
  try {
      ...
      <div class="p-4 space-y-4">
        ...
        <div v-if="group" class="bg-white dark:bg-slate-800 rounded-lg p-4">
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-1 cursor-pointer" @click="copyGroupNumber">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg p-4">
          <div class="flex items-center justify-between mb-3">
            ...
            <button @click="handleViewAllMembers" class="text-sm text-primary hover:text-primary/80 transition-colors">
              查看全部
            </button>
          </div>
          <div class="grid grid-cols-4 gap-4">
            ...
            <button v-if="isAdmin" @click="handleInviteMembers" class="flex flex-col items-center gap-2 hover:opacity-80 transition-opacity">
      ...
      <div class="bg-white dark:bg-slate-800 w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl">
        ...
        <div class="p-4">
          ...
          <div class="mt-4 flex gap-3">
            ...
            >{{ isSavingAnnouncement ? '保存中...' : '保存并发布' }}</button>
          </div>
        </div>
      </div>
    </div>
